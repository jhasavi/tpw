-- =============================================
-- PROFILE ENHANCEMENTS - QUICK MIGRATION
-- Copy and paste this into Supabase SQL Editor
-- =============================================

-- Step 1: Extend profiles table with new columns
...
BEGIN
  ...
  -- Learning style constraint
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'profiles_learning_style_check'
  ) THEN
    ALTER TABLE public.profiles 
    ADD CONSTRAINT profiles_learning_style_check
    CHECK (preferred_learning_style IN ('visual', 'auditory', 'reading', 'kinesthetic', 'mixed'));
  END IF;
END $$;

-- Step 2: Create achievements table (if not exists)
...
-- Step 3: Create course_bookmarks table (if not exists)
...
-- Step 4: Create lesson_bookmarks table (if not exists)
...
-- Step 5: Create learning_streaks table (if not exists)
...
-- Step 6: Create learning_playlists table (if not exists)
...
-- Step 7: Create playlist_items table (if not exists)
...
-- Step 9: Create RLS policies for course_bookmarks
DROP POLICY IF EXISTS "Users can view their own bookmarks" ON public.course_bookmarks;
CREATE POLICY "Users can view their own bookmarks" ON public.course_bookmarks
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own bookmarks" ON public.course_bookmarks;
CREATE POLICY "Users can insert their own bookmarks" ON public.course_bookmarks
  FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete their own bookmarks" ON public.course_bookmarks;
CREATE POLICY "Users can delete their own bookmarks" ON public.course_bookmarks
  FOR DELETE USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own bookmarks" ON public.course_bookmarks;
CREATE POLICY "Users can update their own bookmarks" ON public.course_bookmarks
  FOR UPDATE USING (auth.uid() = user_id);

-- Step 10: Create RLS policies for lesson_bookmarks
DROP POLICY IF EXISTS "Users can view their own lesson bookmarks" ON public.lesson_bookmarks;
...
DROP POLICY IF EXISTS "Users can update their own lesson bookmarks" ON public.lesson_bookmarks;
CREATE POLICY "Users can update their own lesson bookmarks" ON public.lesson_bookmarks
  FOR UPDATE USING (auth.uid() = user_id);

-- Step 11: Create RLS policies for learning_streaks
...
DROP POLICY IF EXISTS "Users can insert their own streak" ON public.learning_streaks;
CREATE POLICY "Users can insert their own streak" ON public.learning_streaks
  FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own streak" ON public.learning_streaks;
CREATE POLICY "Users can update their own streak" ON public.learning_streaks
  FOR UPDATE USING (auth.uid() = user_id);

-- Step 12: Create RLS policies for learning_playlists
DROP POLICY IF EXISTS "Users can view their own playlists" ON public.learning_playlists;
CREATE POLICY "Users can view their own playlists" ON public.learning_playlists
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own playlists" ON public.learning_playlists;
CREATE POLICY "Users can insert their own playlists" ON public.learning_playlists
  FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own playlists" ON public.learning_playlists;
CREATE POLICY "Users can update their own playlists" ON public.learning_playlists
  FOR UPDATE USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete their own playlists" ON public.learning_playlists;
CREATE POLICY "Users can delete their own playlists" ON public.learning_playlists
  FOR DELETE USING (auth.uid() = user_id);

-- Step 13: Create RLS policies for playlist_items
DROP POLICY IF EXISTS "Users can view items from their playlists" ON public.playlist_items;
...
DROP POLICY IF EXISTS "Users can insert items to their playlists" ON public.playlist_items;
...
DROP POLICY IF EXISTS "Users can delete items from their playlists" ON public.playlist_items;
...
CREATE TRIGGER update_streak_on_lesson_progress
  ...
  FOR EACH ROW
  EXECUTE FUNCTION update_learning_streak(NEW.user_id);

DROP TRIGGER IF EXISTS update_profile_completeness_trigger ON public.profiles;
...
-- Step 17: Seed achievements data
...
-- Done!
SELECT '✅ Migration complete! All tables created and achievements seeded.' AS status;
